...
export class AutomationService {
  ...
  async ingestContacts(contacts: GovContact[]) {
    console.log(
      `[Automation] Ingesting ${contacts.length} government contacts...`,
    );
  ...
  async composeApplicationEmail(
    ...
    if (
      !eligibilityResult ||
      !allowedStatuses.includes(eligibilityResult.status)
    ) {
      throw new Error(
        "User is not eligible to draft an application for this program.",
      );
    }
    ...
    const primaryContact =
      ...
      (contacts.length > 0 ? contacts[0].email : "support@agency.gov");
    ...
    if (attachPdf) {
      docsToAttach = docsToAttach.filter(
        (doc) => doc.document_type !== "application_package",
      );
    }

    const applicantName = formatUserName(application.user) || "Applicant";
    const monthlyIncome = familyProfile
      ? decimalToNumber(familyProfile.monthly_income)
      : null;
    ...
    const templateBody =
      `Dear ${program.agency} Representative,\n\n` +
...
The email should be ready to send as-is. End with "MomPlan Automations System" as the sender.`;
    ...
    if (familyProfile) {
      try {
        ...
        console.error(
          "Failed to generate AI email, falling back to template",
          err,
        );
  ...
  async onApplicationStatusChange(applicationId: string, newStatus: string) {
    console.log(
      `[Automation] Hook triggered for Application ${applicationId} -> ${newStatus}`,
    );

    if (newStatus === "submitted") {
      // Future: automatically generate email composition and queue it for review
      console.log(
        `[Automation] Queuing async review tasks for submitted application.`,
      );
    }
  }
  ...
  async processApplication(
    ...
    console.log(
      `[Worker] Processing Apply Now for application: ${applicationId}`,
    );
    ...
    try {
      ...
      const emailData = await this.composeApplicationEmail(
        applicationId,
        userId,
        attachPdf,
        documentIds,
      ...
      console.log(
        `[Worker] Successfully processed Apply Now for application: ${applicationId}`,
      );
    } catch (error: any) {
      console.error(
        `[Worker] Failed to process application ${applicationId}:`,
        error,
      );
